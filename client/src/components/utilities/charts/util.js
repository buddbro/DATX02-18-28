...
export const styleSvg = (style = {}, sourceProps) => {
  ...
  if (sourceProps.fill) {
    style.fill = _.isString(sourceProps.fill)
      ? sourceProps.fill
      : sourceProps.fill.color;
    style.fillOpacity = sourceProps.fill.alpha
      ? sourceProps.fill.alpha / 100
      : 1;
  }
  if (sourceProps.stroke) {
    style.stroke = _.isString(sourceProps.stroke)
      ? sourceProps.stroke
      : sourceProps.stroke.color;
...
export class Options {
  constructor(props) {
    ...
    this.width =
      this.chartWidth + (this.margin.right || 0) + (this.margin.left || 0);
    this.height =
      this.chartHeight + (this.margin.top || 0) + (this.margin.bottom || 0);
  ...
  get legendPosition() {
    return (
      ...
      (this.props.options && this.props.options.legendPosition) ||
      'topLeft'
    );
  }
  get axisX() {
    return (
      this.props.axisX || (this.props.options && this.props.options.axisX) || {}
    );
  }
  get axisY() {
    return (
      this.props.axisY || (this.props.options && this.props.options.axisY) || {}
    );
  }
  get margin() {
    return (
      this.props.margin ||
      (this.props.options && this.props.options.margin) ||
      {}
    );
  }
  get stroke() {
    return (
      this.props.stroke || (this.props.options && this.props.options.stroke)
    );
  }
  get fill() {
  ...
  get label() {
    return (
      this.props.label || (this.props.options && this.props.options.label) || {}
    );
  }
  get animate() {
    return (
      this.props.animate ||
      (this.props.options && this.props.options.animate) ||
      {}
    );
  }
}
